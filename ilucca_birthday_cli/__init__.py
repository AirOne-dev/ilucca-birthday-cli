...
def cli():
    parser = argparse.ArgumentParser(
        description="Affiche les prochains anniversaires.",
        add_help=False,
        usage="python3 %(prog)s [-h] [--next | --all | --today]",
    )
    parser.add_argument(
        "-h",
        "--help",
    ...
    group.add_argument(
        ...
        "--next",
        action="store_true",
        help="Affiche le prochain anniversaire à venir.",
    )
    group.add_argument(
        "-a",
        "--all",
        action="store_true",
        help="Affiche tous les anniversaires à venir.",
    )
    group.add_argument(
        "-t",
        "--today",
        action="store_true",
        help="Affiche tous les anniversaires du jour.",
    )
    ...
    if not any(vars(args).values()):
        ...
        exit()
    print(
        birthday_utils.get_birthdays(
            returnNext=args.next, returnAll=args.all, returnToday=args.today
        )
    )
